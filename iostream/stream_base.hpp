...
#include <iostream/byte_buffer.hpp>
...
inline constexpr std::size_t kStreamBufferSize = 8192;
    ...
    struct Stream
    {
        ...
        virtual Task<Expected<std::size_t>> raw_write(std::span<char const> buffer){
            co_return std::errc::not_supported;
        }
    ...
    struct BorrowedStream
    {
        ...
        Task<Expected<>> getspan(std::span<char> s)
        {
            ...
            while(true)
            {
                ...
                if(end <= mInEnd)
                {
                    p = std::copy(mInBuffer.data() + start,mInBuffer.data() + end,p);
                    mInIndex = end;
                    co_return {};
                }
