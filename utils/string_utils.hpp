...
struct from_string_t<T> {
    std::optional<T> operator()(std::string_view s, int base = 10) const {
        T result;
        auto [p, ec] = // 使用 std::from_chars 将字符串转换为整型
            std::from_chars(s.data(), s.data() + s.size(), result, base);
        if (ec != std::errc()) [[unlikely]] {// 检查转换是否出错
            return std::nullopt; 
        }
        if (p != s.data() + s.size()) [[unlikely]] {// 检查是否完全解析字符串
            return std::nullopt; 
        }
        return result;
    }
...
// 针对浮点型数据特化 from_string_t
template <std::floating_point T>
struct from_string_t<T> {
    std::optional<T>// 返回一个可选浮点类型
...
template <class T>// 声明一个常量表达式 from_string_t 实例
inline constexpr from_string_t<T> from_string;
template <class T = void>
struct to_string_t;

// 特化 to_string_t 以处理 void 类型
template <>
struct to_string_t<void> {
    template <class U>
    void operator()(String &result, U &&value) const {
        to_string_t<std::decay_t<U>>()(result, std::forward<U>(value));// 调用其他 to_string_t 实例
    }
