...
{
    /*
        expected类用于错误处理，包含两种结果：
        执行成功时，返回T的值
        执行失败时：返回U的值
        通过expected类，提供零开销和可读性更强的错误处理
    */
...
# define ZH_ASYNC_EXPECTED_LOCATION \
     , std::source_location const &errLoc = std::source_location::current()
# define ZH_ASYNC_EXPECTED_LOCATION_INIT , mErrorLocation(errLoc)
# define ZH_ASYNC_EXPECTED_LOCATION_MESSAGE \
     , std::string(mErrorLocation.file_name()) + ":" + \
           std::to_string(mErrorLocation.line()) + ": " + \
           mErrorLocation.function_name()
# define ZH_ASYNC_EXPECTED_LOCATION_COPY   , mErrorLocation(that.mErrorLocation)
# define ZH_ASYNC_EXPECTED_LOCATION_ASSIGN mErrorLocation = that.mErrorLocation
# define ZH_ASYNC_EXPECTED_LOCATION_ARG    , mErrorLocation
# define ZH_ASYNC_ERROR_FORWARD(e)         {(e).error(), (e).mErrorLocation}
#else
# define ZH_ASYNC_EXPECTED_LOCATION
# define ZH_ASYNC_EXPECTED_LOCATION_INIT
# define ZH_ASYNC_EXPECTED_LOCATION_MESSAGE
# define ZH_ASYNC_EXPECTED_LOCATION_COPY
# define ZH_ASYNC_EXPECTED_LOCATION_ASSIGN
# define ZH_ASYNC_EXPECTED_LOCATION_ARG
# define ZH_ASYNC_ERROR_FORWARD(e)         e.error()
#endif

public:
    /*
        std::convertible_to 
        用于检查一个类型是否能够隐式或显式转换为另一个类型，并确保两种转换方式的结果等价
        所以这里保证了convertible_to转换成error_code errc in_place_t并且可以转换成T
    */
    ...
    Expected(std::error_code const &ec ZH_ASYNC_EXPECTED_LOCATION) noexcept
        : mErrorCatgory(&ec.category()),
          mErrorCode(ec.value()) CO_ASYNC_EXPECTED_LOCATION_INIT {}

    Expected(std::errc e ZH_ASYNC_EXPECTED_LOCATION) noexcept
...
#undef ZH_ASYNC_EXPECTED_LOCATION
#undef ZH_ASYNC_EXPECTED_LOCATION_INIT
#undef ZH_ASYNC_EXPECTED_LOCATION_MESSAGE
#undef ZH_ASYNC_EXPECTED_LOCATION_COPY
#undef ZH_ASYNC_EXPECTED_LOCATION_ASSIGN
#undef ZH_ASYNC_EXPECTED_LOCATION_ARG
} // namespace zh_async
